...
lst_learning_rate = [1e-3,1e-4,1e-5]  # learning rates
lst_epochs = [100,500,1000]         # number of epochs 
lst_activation = ['relu', 'tanh', 'sigmoid']  # activation functions 
all_results = []


# -----------------------
# Main Loop to Evaluate All Models
# -----------------------
    ...
    for act in lst_activation:
        for lr in lst_learning_rate:
            for epochs in lst_epochs:
                model_path = f"models_parameter/model_{model_name}_{act}_lr{lr}_ep{epochs}_best.pth"
...
df.to_csv("evaluation_results_parameter.csv", index=False)
print("Evaluation results saved to 'evaluation_results_parameter.csv'")
